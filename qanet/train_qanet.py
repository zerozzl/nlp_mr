...
def get_dataset(args, tokenizer, bigram_tokenizer):
    train_dataset = MrDataset('%s/%s/train.json' % (args.data_path, args.task), args.token_type, tokenizer,
                              ...
                              do_pad=True, do_to_id=True, do_train=True, debug=args.debug)
    test_dataset = MrDataset('%s/%s/dev.json' % (args.data_path, args.task), args.token_type, tokenizer,
                             use_bigram=args.use_bigram, bigram_tokenizer=bigram_tokenizer,
                             max_context_len=args.max_context_len, max_question_len=args.max_question_len,
                             do_pad=True, do_to_id=True, do_train=False, debug=args.debug)
...
def main(args):
    ...
    if args.pretrained_model_path is not None:
        ...
        model = QANet(len(token_to_id), args.embed_size,
                      args.conv_kernel_num, args.conv_kernel_size, args.conv_layer_num, args.attn_head_num,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--conv_kernel_size', type=int,
                        default=7)
    parser.add_argument('--conv_layer_num', type=int,
                        default=4)
    parser.add_argument('--attn_head_num', type=int,
                        default=8)
